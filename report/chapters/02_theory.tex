...
Pipeline sử dụng module phát hiện văn bản của \model{PaddleOCR}~\cite{du2020ppocr}; phần nhận dạng tích hợp của PaddleOCR bị tắt. Họ PP-OCR sử dụng bộ phát hiện dựa trên Differentiable Binarization (DB), trong đó bước nhị phân hóa được đưa vào mạng dưới dạng khả vi:
\begin{equation}
...
\model{Donut} (Document Understanding Transformer)~\cite{kim2022donut} là kiến trúc Transformer encoder--decoder ánh xạ trực tiếp điểm ảnh của tài liệu thành chuỗi có cấu trúc mà không cần module OCR trung gian. Trong đồ án, cấu trúc đích dùng các token mở/đóng tương tự XML.
...
\begin{enumerate}
  ...
  \item \textbf{Text Decoder (mBART):} Checkpoint \code{donut-base} dùng decoder thuộc họ mBART, một biến thể đa ngôn ngữ của kiến trúc BART~\cite{lewis2020bart}. Tại mỗi bước thời gian $i$, decoder kết hợp tự chú ý trên các token đã sinh và cross-attention lên biểu diễn thị giác $\mathbf{z}$ để dự đoán token tiếp theo:
  \begin{equation}
...
\model{LayoutXLM}~\cite{xu2021layoutxlm} là mô hình đa phương thức đa ngôn ngữ cho tài liệu có bố cục phong phú. Mô hình mở rộng kiến trúc đa phương thức của LayoutLMv2 bằng backbone văn bản XLM-RoBERTa~\cite{conneau2020xlmr}, nhờ đó tiếp nhận văn bản đa ngôn ngữ cùng thông tin bố cục và ảnh.
...
Cơ chế tự chú ý nhận biết không gian, được kế thừa từ LayoutLMv2, bổ sung các bias vị trí 1D và 2D dựa trên quan hệ tương đối giữa các bounding box của token $i$ và token $j$:
\begin{equation}
